...
def evaluate(encoder, decoder, dataset_path, max_clicks=20,
             thresholds=None, limit=0):
    """Run mask evaluation benchmark.

    Args:
        encoder: Path to encoder.onnx
        decoder: Path to decoder.onnx
        dataset_path: Path to extracted DAVIS dataset
        max_clicks: Maximum clicks per image
        thresholds: IoU thresholds for NoC metric
        limit: Evaluate only first N images (0 = all)
    """
    ...
    enc_session = ort.InferenceSession(
        str(encoder), providers=["CPUExecutionProvider"]
    )
    print(f"Loading decoder: {decoder}")
    dec_session = ort.InferenceSession(
        str(decoder), providers=["CPUExecutionProvider"]
    )
    ...
    # Load dataset
    pairs = load_davis(str(dataset_path))
    if limit > 0:
        pairs = pairs[:limit]
...
def main():
    ...
    evaluate(args.encoder, args.decoder, args.dataset_path,
             args.max_clicks, thresholds, args.limit)
